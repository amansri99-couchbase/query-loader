...
func main() {
	...
	connectionString := "couchbases://private-endpoint.ttidjfols84dkkz7.sandbox.nonprod-project-avengers.com"
	username := "admin"
	...
	selectQueries := []QueryInfo{
		...
		{Name: "Query 3 (All Routes)", Query: `SELECT sourceairport, destinationairport, airline, stops, schedule FROM ` + "`" + bucketName + "`" + `.` + scopeName + `.route LIMIT 10000;`},
		{Name: "Query 4 (Hotels in US)", Query: `SELECT name, city, country, address, description FROM ` + "`" + bucketName + "`" + `.` + scopeName + `.hotel WHERE country = 'United States' LIMIT 10000;`},
	}
